...
- Проверить, что `third_party/curl-android/include` и `third_party/curl-android/libs` есть в git или перенесены отдельным archive.
...
Текущая политика проекта: `third_party/curl-android/include` и `third_party/curl-android/libs` не игнорируются git. Если бинарники нельзя хранить в рабочем git, перенести эту папку отдельным archive и распаковать в тот же путь.
